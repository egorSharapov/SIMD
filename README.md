# Лабораторная работа 1. Изучение SIMD(SSE+AVX) инструкций архитектуры x86-64

## Цель
Изучить возможности оптимизации кода с использовванием intrinsic функций и векторной обработки, применить их для отрисовки множества  Мандельброта и наложения изображений, используя alpha blending.

## В работе используются
Язык программирования C\C++, набор  компиляторов GCC

## Экспериментальная установка

## Ход работы

### Часть первая. Множество Мандельброта.
Множество Мандельброта — множество точек c на комплексной плоскости, для которых рекуррентное соотношение $z \rightarrow z^2 + c$, при z = 0 задаёт ограниченную последовательность. 
Порядок определения, принадлежит ли точка z множеству (традиционно закрашиваемого чёрным цветом) или нет (закрашивается цветом, зависящим от числа итераций) следующий: на каждой итерации вычисляется текущее расстояние — значение модуля |z|, которое затем сравнивается с «критерием бесконечности» (обычно берётся значение, равное 2). 
Алгоритм в своей основе содержит вышеприведенное соотношение и использует комплексную структуру данных
#### Реализация алгоритма (псевдокод)
```C++
struct complex
{
    Re = 0.0;
    Im = 0.0;
};


void mandelbrot (model, max_iterations)
{
    for (i = 0; i < model->height; i++)
    {
    for (j = 0; j < model->width; j++)
        {
            x = Mandelbrot_init.Re_min + Mandelbrot_init.Re_scalar * j;
	    y = Mandelbrot_init.Im_min + Mandelbrot_init.Im_scalar * i;
	    
	    complex z_0 = {x, y};
	    complex z   = {0, 0};
	    
	    counter       = 0;
	    current_index = 0;
	    
	    while (++counter < max_iterations)
            {
                if (z^2 >= 4)
		    break;
    
                z = next_number (z, z0);
            }

            current_index = (j + i*model->width)*4;
            get_color (&model->dots[current_index], counter, max_iterations);
        }
    }
}
```
Размер вычисляемого множества: 1200х1000 пикселей

#### Результаты измерений (время усреднено для трех запусков тестирующей программмы)

| Версия      | Флаги оптимизации | Число запусков | Время исполнения (us) |
| ------      | :---------------: | :------------: | --------------------: |
| Scalar      | нет               | 128            |       175 273 827 884 | 
| Scalar      | -О3               | 512            |        60 503 953 584 | 
| Scalar      | -О3 -fprofile-use | 512            |        59 579 548 759 |
| Scalar      | -Оfast            | 512            |        60 323 356 279 |
| Vector(sse) | нет               | 128            |        28 040 940 225 | 
| Vector(sse) | -О3               | 512            |        21 802 432 639 |
| Vector(sse) | -О3 -fprofile-use | 512            |        21 059 302 425 |
| Vector(sse) | -Оfast            | 512            |        20 119 211 444 |
| Vector(avx) | нет               | 128            |        18 040 940 225 | 
| Vector(avx) | -О3               | 512            |         8 903 196 401 |
| Vector(avx) | -О3 -fprofile-use | 512            |         8 609 302 425 |
| Vector(avx) | -Оfast            | 512            |         8 525 795 565 |
#### Графики
<img src="/plots/plot4.png" style="height: 370px;"/>
<img src="/plots/plot5.png" style="height: 370px;"/>
Результаты показывают, что без оптимизаций компилятора векторная обработка (sse) оказывается в ~6.25 раз быстрее скалярной. С использованием флага `-O3` векторная обработка (sse) оказывается быстрее скалярной в ~3 раза, a векторная (avx) обработка быстрее скалярной в ~7 раз, при этом оптимизированная версия векторной обработки оказывается быстрее неоптимизированной в 22.4 раза. 

### Часть вторая. Alpha blending.
Испольузется 32-битный (с альфа каналом) формат bmp без сжатия.
Алгоритм использует только целочисленные операции благодаря битовым сдвигам и разделению цвета на две компоненты (красная+синия и зеленая)

#### Реализация алгоритма (псевдокод)
```C++
alpha = foreground >> 24;
    
red_blue  = background & 0x00FF00FF;
green     = background & 0xFF00FF00;

red_blue += ((foreground & 0x00FF00FF - red_blue) * alpha) >> 8;
green    += ((foreground & 0xFF00FF00 -    green) >> 8) * alpha;
   
out = (red_blue & 0x00FF00FF)  | (green & 0xFF00FF00);
```

```C++
foreground_pxl = _mm256_loadu_si256 ((__m256i *) (foreground->pixels + x + y*width));
background_pxl = _mm256_loadu_si256 ((__m256i *) (background->pixels + x + y*width));
	
alpha = SRL8 (foreground_pxl, twenty_four);
            
red_blue = AND8 (background_pxl, rb_mask);
green    = AND8 (background_pxl, g_mask);
            
red_blue = ADD8 (red_blue, SRL8 (MULLO8 (SUB8 (AND8 (foreground_pxl, rb_mask), red_blue), alpha), eight));
green    = ADD8 (green,    SRL8 (MULLO8 (SUB8 (AND8 (foreground_pxl,  g_mask),    green), alpha), eight));
            
out_pxl = OR8 (AND8 (red_blue, rb_mask), AND8 (green, g_mask));

_mm256_storeu_si256 ((__m256i *) (out->pixels + x + y*width),out_pxl);
```

Размер изображения: 512x512

#### Результаты измерений (время усреднено для трех запусков тестирующей программмы)

| Версия      | Флаги оптимизации | Число запусков | Время исполнения (us) |
| ------      | :---------------: | :------:       | ------: |
| Scalar      | нет               | 2048           | 3 238 683 753 | 
| Vector(sse) | нет               | 2048           | 2 580 843 021 |
| Vector(avx) | нет               | 2048           | 1 406 541 603 |
| Scalar      | -О3               | 2048           |   802 411 367 |
| Vector(sse) | -О3               | 2048           |   240 692 377 |
| Vector(avx) | -O3               | 2048           |   134 546 940 |
| Scalar      | -Оfast            | 2048           |   800 456 005 |
| Vector(sse) | -Оfast            | 2048           |   257 381 083 |
| Vector(avx) | -Оfast            | 2048           |   119 893 228 |
#### Графики
<img src="/plots/plot2.png" style="height: 370px;"/>
Замеры показывают, что без оптимизаций компилятора векторная обработка (sse) быстрее скалярной всего в 
1.28 раза или в 2.28 раза в случае avx2. При этом с использованием оптимизаций gcc прирост при использовании векторной обработки по сравнению со скалярной составляет ~7 порядков. Также можно отметить, что не всегда использование агрессивной оптимизации математических выражений позволяет получить прирост скорости работы. Например, в случае sse с флагами `-O3 -ffast-math` программа стала медленнее на 6.67%, по сравнению с `-O3`

Также был проведен тест другого алгоритма alpha blending, использющего shuffle-инструкции.

#### Реализация shuffle-алгоритма (псевдокод)
```C++
p0_64    =   unpacklo_epi8 (background, zero);
p1_64    =   unpackhi_epi8 (background, zero);

alpha_lo =   shuffle_epi8  (foreground,  allo);
alpha_hi =   shuffle_epi8  (foreground,  alhi);

alpha1_l =   xor_ps        (ag_mask, alpha_lo);
alpha1_h =   xor_ps        (ag_mask, alpha_hi);
mul_lo   =   mulhi_epu16   (p0_64, alpha1_l); 
mul_hi   =   mulhi_epu16   (p1_64, alpha1_h);

f_alpha  =   packus_epi16  (mul_lo, mul_hi);
   
p0_64    =   unpacklo_epi8  (foreground_pxl, zero); 
p1_64    =   unpackhi_epi8  (foreground_pxl, zero);
mul_lo   =   mulhi_epu16    (p0_64, alpha_lo);
mul_hi   =   mulhi_epu16    (p1_64, alpha_hi);
out_pxl  =   adds_epi8      (packus_epi16 (mul_lo, mul_hi), f_alpha);
```
#### Результаты измерений (время усреднено для трех запусков тестирующей программмы)

| Версия      | Флаги оптимизации | Число запусков | Время исполнения (us) |
| ------      | :---------------: | :------:       |               ------: |
| Shuffle(avx)| -O3               | 8192           |           766 238 319 | 
| Origin(avx) | -O3               | 8192           |           783 970 327 |
| Origin      | -O3               | 8192           |         5 256 041 881 |
#### Графики
<img src="/plots/plot3.png" style="height: 370px;"/>
Результаты показывают, что оригинальный алгоритм проигрывает shuffle-алгоритму на 3%, при этом от запуска к запуску эта разница в скорости работы нивелируется

### Заключение
- В данной работе мы выяснили, что в среднем векторная обработка работает быстрее скалярной в среднем в 3-4 раза в случае sse и в 7-8 раз в случае avx.
- С флагами `-O3 -ffast-math` и использованием avx2 инструкций можно добиться ускорения до 7 раз. 
- Из первой части можно сделать вывод, что наибольшей эффективности программы можно достигнуть, комбинируя использование векторной обработки и оптимизаций компилятора. 
- Также важно отметить, что не всегда более агрессивные флаги оптимизации компилятора ведут к ускорению работы программы, а в некоторых случаях даже ломают нормальное поведение программы. 


